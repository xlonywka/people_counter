...
Room::Room(byte pinSensor1, byte pinSensor2, byte pinRelay, byte mode)
{   
...
Room::Room(byte pinSensor1, byte pinSensor2, byte pinRelay, byte mode, byte pinHandMode)
{   
    _pinSensor1 = pinSensor1;
    _pinSensor2 = pinSensor2;
	_mode = mode;
...
Room::Room(byte pinSensor1, byte pinSensor2, byte pinRelay, byte mode, byte pinHandMode, byte pinPhotoSensor, int photoSensorThreshold)
{  
    _pinSensor1 = pinSensor1;
    _pinSensor2 = pinSensor2;
	_mode = mode;
...
byte Room::statusControl()
{
     if(_handModeInit == 1)
     {
        if(digitalRead(_pinHandMode) == 0) //если с кнопки пришел 0, включаем "ручной" режим
        {
			if(mode == "light")
			{
				statusRoom(0);     //выключить свет в этой комнате

			}
			else 
			{
				statusRoom(1)      //включить вентиляцию/кварцевание
			}
        ...
        else
        {
          if (count_people_val() > 0)  //если есть люди в комнате
          {
              if(mode == "light")
			{
				statusRoom(1);     //включить свет в этой комнате

			}
			else 
			{
				statusRoom(0)      //выключить вентиляцию/кварцевание
			}
          }
          else
          {
...
void Room::statusRoom(bool statusOn)
{
    _statusOn = statusOn;
    if(_photoSensorInit == 1)
    {
      ...
      if(_onDebug == 1)
      {
        ...
        if(_darkLight == 1)
            Serial.println(" Status in Room: "+ String(_statusOn));
        else
            Serial.println(" Status in Room: 0");
      }
      ...
      if(_darkLight == 1)
      {
          digitalWrite(_pinRelay, _statusOn);
      }
    }
    else
    {
